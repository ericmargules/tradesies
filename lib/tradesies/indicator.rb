module Tradesies
	class Indicator
		...
		def bands(data, period) #Bollinger Bands
			...
			bbands[:upper_band] = bbands[:middle_band] + band_diff
			bbands[:lower_band] = bbands[:middle_band] - band_diff
			bbands
		end

		private
		
		def std_dev(data, period) # Standard Deviation
			sub1 = subset(data, period)
			avg = average(subset(data, period))
			sq_dev = sub1.map{ |point| (point - avg) * (point - avg) }
			Math.sqrt(average(sq_dev))
		end
		
		def mean_dev(data, period) # Mean Deviation
		...
		def average(data)
			data.inject(:+) / Float(data.length)
		end

		# EMA/SMA/CCI/BBands info from http://stockcharts.com/school/doku.php?id=chart_school:technical_indicators:moving_averages
		...
		# BBands test data  [53.73,53.87,53.85,53.88,54.08,54.14,54.50,54.30,54.40,54.16]
